...
# ✅ النسخة المصححة - حل مشكلة حساب المخزون
...
class StockCardWizard(models.TransientModel):
    ...
    def _get_real_product_ids(self):
        """استخراج الـ IDs الحقيقية من product_ids"""
    ...
    def _get_filtered_products(self):
        ...
        domain = [('active', '=', True), ('type', '=', 'product')]  # ✅ فقط المنتجات القابلة للتخزين
    ...
    # ============================================================
    # ✅ دوال حساب المخزون المصححة
    # ============================================================

    def _get_current_stock_from_quants(self, product, locations):
        """
        ✅ الحصول على الرصيد الفعلي الحالي من stock.quant
        هذه هي الطريقة الصحيحة للحصول على المخزون الفعلي
        """
        domain = [
        ...
        if locations:
            ...
            # فقط الـ internal locations
            domain.append(('location_id.usage', '=', 'internal'))

        quants = self.env['stock.quant'].search(domain)
        return sum(quants.mapped('quantity'))

    def _calculate_stock_at_date(self, product, locations, target_date):
        """
        ✅ حساب المخزون في تاريخ معين
        نبدأ من المخزون الحالي ونرجع للخلف
        """
        # الرصيد الحالي
        current_qty = self._get_current_stock_from_quants(product, locations)

        if not target_date:
            return current_qty

        # نحسب كل الحركات من target_date لحد النهاردة ونعكسها
        ...
        for move in moves:
            if locations:
                ...
                from_in = move.location_id.id in location_ids
                to_in = move.location_dest_id.id in location_ids

                # عكس الحركة - لأننا نرجع للخلف
                if to_in and not from_in:
                    ...
                    # كانت issue - نضيفها
                    adjustment += move.product_uom_qty
                # لو internal transfer بين locations في الفلتر - مش بتأثر
            else:
    ...
    def _get_movements_between_dates(self, product, locations, date_from, date_to):
        """
        ✅ الحصول على الحركات بين تاريخين
        """
        domain = [
            ('product_id', '=', product.id),
            ('state', '=', 'done'),
        ...
        if date_to:
            end_datetime = fields.Datetime.to_datetime(date_to).replace(hour=23, minute=59, second=59)
    ...
    def _calculate_receipt_issue_from_move(self, move, locations):
        """
        ✅ حساب كمية الاستلام والصرف من حركة واحدة
        """
        receipt_qty = 0
        issue_qty = 0
        ...
        if locations:
            ...
            # لو from_in و to_in كلاهما True (internal transfer) - مش بنحسبها
        else:
            from_internal = move.location_id.usage == 'internal'
            to_internal = move.location_dest_id.usage == 'internal'
    ...
    def _get_product_stock_card(self, product, locations):
        """Stock Card for a single product"""
        # ✅ حساب الرصيد الافتتاحي بالطريقة الصحيحة
        if self.date_from:
            opening_balance = self._calculate_stock_at_date(product, locations, self.date_from)
        else:
            opening_balance = 0  # لو All Time نبدأ من صفر

        # الحصول على الحركات
        moves = self._get_movements_between_dates(product, locations, self.date_from, self.date_to)

        movements = []
        running_balance = opening_balance

        for move in moves:
            receipt_qty, issue_qty = self._calculate_receipt_issue_from_move(move, locations)

            # تخطي الحركات اللي مش بتأثر (مثل internal transfers)
            if receipt_qty == 0 and issue_qty == 0:
                continue
        ...
        try:
            ...
            if hasattr(tmpl, 'item_short_code'):
                item_short_code = tmpl.item_short_code or ''
            if hasattr(tmpl, 'remark_type_id') and tmpl.remark_type_id:
    ...
    # ============================================================
    # ✅ Stock Value Report - المصحح
    # ============================================================

    def _get_stock_value_data(self):
        """Stock Value Report Data - مصحح"""
        _logger.info("=" * 80)
        _logger.info("Starting Stock Value Report - FIXED VERSION")
        ...
        if location_ids:
            ...
            locations = False  # كل الـ internal locations
        ...
        for product in products:
            ...
            # ✅ الرصيد الحالي الفعلي من quants
            current_qty = self._get_current_stock_from_quants(product, locations)
            _logger.info(f"  Current qty from quants: {current_qty}")

            # ✅ حساب الرصيد الافتتاحي
            if self.date_from:
                opening_qty = self._calculate_stock_at_date(product, locations, self.date_from)
            else:
                opening_qty = 0
            _logger.info(f"  Opening qty: {opening_qty}")
            ...
            moves = self._get_movements_between_dates(product, locations, self.date_from, self.date_to)
            for move in moves:
                receipt, issue = self._calculate_receipt_issue_from_move(move, locations)
                receipts_qty += receipt
                issues_qty += issue

            _logger.info(f"  Receipts: {receipts_qty}, Issues: {issues_qty}")

            # ✅ الرصيد الختامي = الافتتاحي + الاستلامات - المنصرف
            closing_qty = opening_qty + receipts_qty - issues_qty
            _logger.info(f"  Calculated closing qty: {closing_qty}")

            # ✅ التحقق من الرصيد الختامي مع الفعلي (لو date_to هو اليوم)
            if self.date_to and self.date_to >= fields.Date.today():
                if abs(closing_qty - current_qty) > 0.01:
                    _logger.warning(f"  ⚠️ Mismatch! Calculated: {closing_qty}, Actual: {current_qty}")
                    # نستخدم الرصيد الفعلي
                    closing_qty = current_qty

            # التكلفة والقيمة
            cost = product.standard_price
            opening_value = opening_qty * cost
            closing_value = closing_qty * cost
    ...
    def _generate_stock_value_html(self, data):
        ...
        for i, p in enumerate(data.get('products', []), 1):
            # ✅ تلوين الصف حسب الرصيد
            row_style = ''
            if p['closing_qty'] < 0:
                row_style = 'background-color: #ffebee;'  # أحمر فاتح للرصيد السالب
            elif p['closing_qty'] == 0:
                row_style = 'background-color: #fff3e0;'  # برتقالي فاتح للرصيد صفر
