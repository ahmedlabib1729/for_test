...
class BookingRegistrations(models.Model):
    ...
    emirates_id = fields.Char(
        ...
        required=True,
        tracking=True,
        help='أدخل رقم الهوية بالصيغة: 784-YYYY-XXXXXXX-X'
    )
    ...
    def _check_emirates_id_format(self):
        ...
        for record in self:
            if record.emirates_id:
                # إزالة المسافات
                clean_id = record.emirates_id.replace(' ', '').strip()

                # التحقق من الصيغة
                if not emirates_id_pattern.match(clean_id):
                    raise ValidationError(
                        'رقم الهوية الإماراتية غير صحيح! '
                        'يجب أن يكون بالصيغة: 784-YYYY-XXXXXXX-X'
                    )

    @api.constrains('emirates_id', 'program_ids', 'workshop_id', 'department_id')
    def _check_duplicate_registration(self):
        """منع التسجيل المكرر لنفس الشخص في نفس البرنامج أو الورشة"""
        for record in self:
            ...
            if record.booking_mode == 'workshop' and record.workshop_id:
                ...
                domain.append(('workshop_id', '=', record.workshop_id.id))
                existing = self.search(domain)
                if existing:
                    raise ValidationError(
                        f'لا يمكن التسجيل! رقم الهوية {record.emirates_id} '
                        f'مسجل بالفعل في ورشة {record.workshop_id.name}'
                    )

            elif record.booking_mode == 'programs' and record.program_ids:
                # التحقق من عدم التسجيل في نفس البرامج
                for program in record.program_ids:
                    program_domain = domain + [('program_ids', 'in', program.id)]
                    existing = self.search(program_domain)
                    if existing:
                        raise ValidationError(
                            f'لا يمكن التسجيل! رقم الهوية {record.emirates_id} '
                            f'مسجل بالفعل في برنامج {program.name}'
                        )
    ...
    def _create_member_and_subscription(self):
        ...
        subscription_vals = {
            ...
            'headquarters_id': self.headquarters_id.id,
            'department_id': self.department_id.id,
            'amount': 0.0,  # سيتم تحديثه لاحقاً
        }

        # ⭐ تحديد نوع الاشتراك والمبلغ حسب نوع الحجز
        if self.booking_mode == 'workshop' and self.workshop_id:
            subscription_vals.update({
                'workshop_id': self.workshop_id.id,
                'amount': self.workshop_id.price if not self.workshop_id.is_free else 0.0,
        ...
        # ⭐ تحديد الحالة الأولية للاشتراك
        if self.booking_mode == 'workshop' and self.workshop_id and self.workshop_id.is_free:
            # ورشة مجانية - إنشاء الاشتراك كـ active مباشرة
            subscription_vals['state'] = 'active'
            _logger.info(f"Creating active subscription for free workshop {self.workshop_id.name}")
        else:
            # ورشة مدفوعة أو برامج - إنشاء كـ confirmed
            subscription_vals['state'] = 'confirmed'
    ...
    # استبدل دالة action_confirm كاملة
    def action_confirm(self):
        ...
        if self.state == 'draft':
            ...
            # ⭐ التفريق بين الورش المجانية والمدفوعة
            if self.booking_mode == 'workshop' and self.workshop_id and self.workshop_id.is_free:
                # ورشة مجانية - لا نحتاج فاتورة
                self.state = 'confirmed'

                # تفعيل الاشتراك مباشرة مع تجاوز الـ constraints
                if self.subscription_id and self.subscription_id.state == 'confirmed':
                    try:
                        # ⭐ استخدام with_context لتجاوز validation الدفع
                        self.subscription_id.with_context(
                            skip_payment_check=True,  # تجاوز التحقق من الدفع
                            force_activate=True  # إجبار التفعيل
                        ).action_activate()

                        _logger.info(f"Free workshop subscription activated for booking {self.id}")
                    except Exception as e:
                        _logger.error(f"Error activating free workshop subscription: {str(e)}")
                        # حتى لو فشل التفعيل، نكمل العملية
                        pass

                # تغيير الحالة إلى معتمد مباشرة
                self.state = 'approved'

                # إضافة رسالة في chatter
                self.message_post(
                    body="تم التسجيل في ورشة مجانية - تم التفعيل تلقائياً بدون فاتورة",
                    subject="تسجيل في ورشة مجانية"
                )
            else:
                # ورشة مدفوعة أو برامج - السايكل الطبيعي
                if self.department_type == 'ladies':
                    self._create_invoice()

                self.state = 'confirmed'

    @api.depends('booking_mode', 'department_booking_price', 'workshop_price', 'workshop_id.is_free')
    def _compute_final_price(self):
        """حساب السعر النهائي حسب نوع الحجز"""
        for record in self:
            if record.booking_mode == 'workshop' and record.workshop_id:
                # إذا كانت الورشة مجانية، السعر = 0
                if record.workshop_id.is_free:
                    record.final_price = 0.0
                else:
                    record.final_price = record.workshop_price
            else:
                # البرامج - السعر العادي
    ...
    def _onchange_workshop_id(self):
        """التحقق من توفر مقاعد في الورشة وعرض معلومة المجانية"""
        if self.workshop_id:
            # التحقق من المقاعد
            if self.workshop_id.available_seats <= 0:
                self.workshop_id = False
                return {
                    'warning': {
                        'title': 'الورشة ممتلئة',
            ...
            # ⭐ جديد: عرض رسالة للورش المجانية
            if self.workshop_id.is_free:
                return {
                    'warning': {
                        'title': '✓ ورشة مجانية',
                        'message': f'هذه الورشة مجانية! سيتم تفعيل الاشتراك مباشرة بعد التسجيل.\n'
                                   f'المقاعد المتاحة: {self.workshop_id.available_seats}'
                    }
                }
    ...
    # استبدل دالة _create_invoice كاملة
    def _create_invoice(self):
        ...
        # ⭐ إضافة: لا نحتاج فاتورة للورش المجانية
        if self.booking_mode == 'workshop' and self.workshop_id and self.workshop_id.is_free:
            return False
